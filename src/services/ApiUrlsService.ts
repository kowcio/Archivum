...
export default class ApiUrlsService {
    ...
    private static readonly URL_PARAMS = {
        KONTO_FINANSOWE: '',
        get DATA_OD() {
            return ApiUrlsService.getDateParams().DATA_OD
        },
        get DATA_DO() {
            return ApiUrlsService.getDateParams().DATA_DO
        },
        DOKUMENT_IDENT: '',
    ...
    public static getUrl(type: keyof typeof ApiUrlsService.urls, params: { [key: string]: string | number }) {
    ...
    public static getFinanseUrl(kontoFinansowe: string, data_od: string = this.URL_PARAMS.DATA_OD, data_do: string = this.URL_PARAMS.DATA_DO) {
    ...
    public static getDokumentyUrl(ident: string, czyDlaMenuWspolne: string = 'false', doFakturyWspolne: string = 'false') {
