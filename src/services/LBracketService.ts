 ...
 *   (internal) fetchFaviconDataUrl      → fetch favicon URL and convert to data URL
 *   (internal) renderLBracketDataUrl    → render favicon + L-bracket via OffscreenCanvas
 *   (internal) applyLBracketPageScript  → page-injected function to apply bracket
 *   (internal) removeLBracketPageScript → page-injected function to remove bracket
 */
...
export class LBracketService {
    ...
    static async renderBracket(rawFaviconUrl: string | undefined, color: string): Promise<string> {
        const faviconDataUrl = rawFaviconUrl
            ? await LBracketService.fetchFaviconDataUrl(rawFaviconUrl)
            : null
        return LBracketService.renderLBracketDataUrl(faviconDataUrl, color)
    }
    ...
    static async applyBracket(tabId: number, rawFaviconUrl: string | undefined, color: string): Promise<string> {
        ...
        await browser.scripting.executeScript({
            target: { tabId },
            func: LBracketService.applyLBracketPageScript,
            args: [renderedUrl],
        })
        return renderedUrl
    }
    ...
    static async removeBracket(tabId: number): Promise<void> {
        await browser.scripting.executeScript({
            target: { tabId },
            func: LBracketService.removeLBracketPageScript,
            args: [],
        })
    }

    // ─────────────────────────────────────────────────────────────────────────────
    // Internal helpers (previously in TabDots)
    // ─────────────────────────────────────────────────────────────────────────────
     ...
     */
    private static async fetchFaviconDataUrl(url: string): Promise<string | null> {
     ...
     */
    private static async renderLBracketDataUrl(faviconDataUrl: string | null, color: string): Promise<string> {
        const SIZE           = 32
        const BORDER         = 4.5
