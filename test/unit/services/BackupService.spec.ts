...
describe('BackupService', () => {
  ...
  afterEach(() => {
   ...
   it('should auto-backup tabs and groups', async () => {
     ...
     const backup = callArg['archivum:tab_backup'] as Backup
  ...
  it('should handle missing tabGroups gracefully (Firefox)', async () => {
   ...
   it('should handle errors gracefully', async () => {
     mockBrowser.tabs.query.mockRejectedValueOnce(new Error('Query failed'))

     const consoleSpy = vi.spyOn(console, 'error')

     // Should not throw, just log error
     await expect(BackupService.backupTabs()).rejects.toThrow('Query failed')
