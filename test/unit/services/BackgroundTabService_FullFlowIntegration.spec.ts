...
describe('BackgroundTabService', () => {
  ...
  describe('E2E Scenario: Grouping → Ungrouping → Backup → Restore', () => {
    it('should handle complete grouping lifecycle with 10 tabs in 3 groups', async () => {
      ...
      const classifications: Array<{ tabId?: number; url?: string; days: number; classification: AgeClassification }> = []
      ...
      console.log(`  ✓ Plugin creates max ${activeLevels.length} groups (indices 0-${maxPluginIndex})`)
      ...
      Object.entries(groupsByAgeIndex).forEach(([levelIdx, items]) => {
        const level = levelIdx === '0' ? 'Fresh' : activeLevels[parseInt(levelIdx) - 1]?.label ?? '?'
      ...
      console.log(`  ✓ ungroupAllTabs() completed: ${ungroup1Result === undefined ? 'void' : 'returned'} (no-op in fakeBrowser)`)
      ...
      activeLevels.forEach((level, idx) => {
        console.log(`   [Group Index ${idx}] "${level.label}" (${level.days}d) - Color: ${level.color}`)
      })

      console.log('\n📌 TAB CLASSIFICATION & GROUPING:')

      classifications.forEach((c, position) => {
        const groupLabel = c.classification.isFresh ? 'Fresh (ungrouped)' : activeLevels[c.classification.index - 1]?.label ?? 'Unknown'
        console.log(
      ...
      const uniqueGroupIndices = [...new Set(classifications.map(c => c.classification.index))].sort((a, b) => a - b)
